...
```json filename="example_contract.json" {8, 10}
{
  consumer: {
    name: "service_1",
  },
...
In the context of contract test, services are classified as either a consumer or a provider.
In practice, any one service may assume the role of a consumer or a provider in relation to another service.
...
In practice, a single service can be both a consumer and a provider in different situations (but ideally not with respect to the same other service).
...
Contract tests behave more like unit tests or service tests, and can be run quickly, cheaply, and early on in the CI/CD pipeline.
...
As mentioned before, contract tests do not eliminate the need for integration and E2E tests.
...
Instead, contract tests reduce the *quantity* of broadly-scoped tests that are needed in CI/CD, and in some cases allow integration and E2E tests to be run separately from the deployment cycle.
...
Methodologies are grouped into three categories according to how they determine the details of the contract.
...
After the provider API is implemented, the requests in the contract are *replayed* against the API (a process known as **provider verification**) and the responses are compared with those specified in the contract.
If the provider service sends the correct response for each request, the test passes.
...
On the other hand, the consumer-driven approach has a critical drawback.
