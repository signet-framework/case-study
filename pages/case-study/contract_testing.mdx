...
Instead, contract tests reduce the *quantity* of broadly-scoped tests that an application needs.
Ideally, broadly-scoped tests can even be moved out of the CI/CD pipeline and run in a periodic manner that is decoupled from the deployment life-cycle of any given team.
